...
class CaptureForPerception(py_trees.behaviour.Behaviour):
    ...
    def initialise(self) -> None:
        ...
        if self._latest_frame() is not None:
            self._last_index = self._latest_frame().index
            self._tick = 0
    ...
    def update(self) -> py_trees.common.Status:
        ...
        """
        if self._latest_frame() is not None and self._latest_frame().index != self._last_index:
            self._publisher.publish_image(self._latest_frame())
            self._publisher.publish_status({"phase": "capture", "frame_index": self._latest_frame().index})
